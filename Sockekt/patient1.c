...
int main() {
	...
	if (strcmp(feedBack, "success") == 0) {//Compare the message returned by server.
		...
		if (strcmp(port, "notavailable") == 0) {
			printf(
					"Phase 2: The requested appointment from Patient 1 is no available. Exiting...\n");
			printf("\n");
		...
		if (UDP) {
			...
			fpUDP = fopen("patient1insurance.txt", "r");
			//already open the file and read from it.
			...
			printf(
					"In Phase 3: Patient 1 has a dynamic UDP port number %s and IP address %s.\n",
