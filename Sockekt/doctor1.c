...
#define PORT 41629   //The port on which to listen for incoming data
...
int main(void) {
	...
	fclose(fp);
/**
 * Save insurance plan and cost seperately
 *
 */
	char * pch;
	int row = 0;
	...
	//create a UDP socket
	if ((s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) {//reused code;
		die("socket");
	}
	...
	while (1) {
		...
		printf(
				"Phase 3: Doctor 1 receives the request from the patient with port number %d and name %s with the insurance plan %s.\n",
